...
namespace Drupal\localgov_election_reporting_social_post\Form;
...
class AreaVoteSocialPostForm extends FormBase
{
   ...
   *
   * @var TwitterPostManagerInterface
   */
   ...
   * @param TwitterPostManagerInterface $twitterPostManager
  ...
  {
    $this->config = $configFactory->get('localgov_election_reporting_social_post.settings');
  ...
  {
    return 'localgov_election_reporting_social_post_area_vote_social_post';
  }
  ...
  public function preview($form, FormStateInterface $form_state)
  {
    ...
    $form_state->set('original_data', ['account' => $form_state->getValue('account'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void
  {
    ...
    foreach ($accounts as $account) {
      ...
      // This is a bit hmm. Can return true even if the tweet returns 403. We only get a bool so not much we can do
      // E.g. duplicate tweets return true but Twitter won't actually post the tweet if it's a duplicate.
      if ($post) {
